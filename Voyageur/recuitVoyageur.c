                          ...
                          Chemin  cheminX, int amplitude)
{
	*cheminY = cheminX;
	int i;
	double val_changement;
	int choix, val1, val2, tmp;
	
	//Entier ampliture = valeur autorisee entre le chemin X et le noeud voisin Y.
	//Compte en nb de transfo.
	//Ampli de 3 -> soit a 1, 2 ou 3 transformations, tire aleatoirement
	val_changement = myRandomMinMax(1, amplitude);
	
	//Tirage aleatoire entre 1 et amplitude pour avoir nb changements
	for(i=0;i<val_changement;i++)
	{
		choix = myRandomMinMax(0, 1);
		 ...
		 switch(choix)
		 {
		 	case 0:
				 ...
				 val1 = myRandomMinMax(0, (cheminY->nbVilles)-1);
				 val2 = myRandomMinMax(0, (cheminY->nbVilles)-1);
				 
				 while(val1 == val2){
				 	val2 = myRandomMinMax(0, (cheminY->nbVilles)-1);
				 }
				 	
				 tmp = cheminY->parcours[val2];
				 cheminY->parcours[val2] = cheminY->parcours[val1];
				 cheminY->parcours[val1] = tmp;
...
{
//Prends en param un chemin, renvoie longueur chemin avec calculCoutChemin()
...
double g(void) {
 ...
 #endif
 	return T-alpha;        /* on decroit le temperature en utilisant alpha */
}
